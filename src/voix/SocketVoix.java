 ...
 */
public class SocketVoix extends Thread{
    ...
    ConversationVoix conv;

    /**
     * Constructeur
     */
    // Socket de connexion entrante
    public SocketVoix(Socket socket){
        // On créer l'identifiant unique
        id = ++ServerVoix.uniqueID;
        ...
        conv = new ConversationVoix(this);
        executor.execute(conv);
    }

    // Socket de connexion sortante
    public SocketVoix(String server, int port, String username){
        this.ip = server;
        this.port = port;
    ...
    public void run(){
        ...
        while (keepGoing) {
            ...
            try {
                cm = (texte.Message) sInput.readObject();
            } catch (IOException e) {
            ...
            switch (cm.getType()) {
                ...
                case texte.Message.MESSAGE:
                    System.out.println(usernameClient + " : " + message);
                    break;
                case texte.Message.LOGOUT:
    ...
    // Envoyer un message
    protected boolean writeMsg(texte.Message msg) {
        // Si le client est toujours connecté
        if(!socket.isConnected()) {
            System.out.println("?");
            close();
